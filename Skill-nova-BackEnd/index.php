...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    else if (isset($_GET['action']) && $_GET['action'] === 'getStudents') { 
        $data = $controller->getStudents();
        echo json_encode($data);
        exit();
    }
    ...
    else if(isset($_GET['answerRoomID'])) {
        $roomID = intval($_GET['answerRoomID']);
        $data = $controller->getAllQuestionsAnswers($roomID);
        echo json_encode($data);
        exit();
    } 
    else if(isset($_GET['action']) && $_GET['action'] === "Allrooms") {
